...
function sumOfCodes(str) {
  ...
  return newStr.reduce((sum, value) => sum + value.charCodeAt(), 0);
}
...
function formatTime(minutes, seconds) {
  const timeM = minutes.toString().padStart(2, '0');
  const timeS = seconds.toString().padStart(2, '0');
  return `${timeM}:${timeS}`;
}
...
function isPalindrome(str) {
  let temp = str.trim().toLowerCase().replaceAll(' ', '');
  ...
  const arr = str.split('').map((value) => {
    if (code.indexOf(value) !== -1) {
      if (value === value.toLowerCase())
        return code[(code.indexOf(value) + 13) % code.length].toLowerCase();
      return code[(code.indexOf(value) + 13) % code.length].toUpperCase();
    }
    return value;
  });
  return arr.join('');
